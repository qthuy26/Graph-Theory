...
List breath_first_search(Graph *G, int x, int parent[]){
    ...
    while(!empty(&frontier)){
        int s = top(&frontier); pop(&frontier);
        push_back(&list_bfs, s);
        List list = neighbor(G, s);
    ...
    for(i = 1; i <= G.n; i++){
        if(mark_bfs[i] == 0){
            ...
            for(j = 1; j <= bfs.size; j++){
                int k = element_at(&bfs, j);
                mark_bfs[k] = 1;        // Danh dau dinh da duoc duyet
            }
        }
    }
    for(i=1; i<=G.n; i++)
